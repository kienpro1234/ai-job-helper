...
import { Textarea } from "@/components/ui/textarea";
import { Input } from "@/components/ui/input";
import { createResume, improveWithAI, updateResume } from "@/actions/resume";
import { EntryForm } from "./entry-form";
import useFetch from "@/hooks/use-fetch";
import { useUser } from "@clerk/nextjs";
import { resumeSchema } from "@/app/lib/schema";
...
export default function ResumeBuilder({
  ...
  }, [saveResult, saveError, isSaving, router]);
  ...
  const getContactMarkdown = () => {
    ...
    if (contactInfo?.linkedin) {
      const url = contactInfo.linkedin.startsWith("http")
        ? contactInfo.linkedin
        : `https://${contactInfo.linkedin}`;
      parts.push(
    ...
    if (contactInfo?.twitter) {
      const url = contactInfo.twitter.startsWith("http")
        ? contactInfo.twitter
        : `https://${contactInfo.twitter}`;
      parts.push(
        `🐦 <a href="${url}" target="_blank" rel="noopener noreferrer">Twitter/X</a>`
      );
    }

    return `<h1 class="resume-name">${
      contactInfo?.displayName || user?.fullName || "Your Name"
    }</h1>\n<div class="contact-info">${parts.join(
      "<span>&nbsp;|&nbsp;</span>"
    )}</div>`;
  };
  ...
  const getCombinedContent = () => {
    ...
    // Sử dụng `\n\n` để tạo đoạn mới giữa tiêu đề và nội dung.
    const createSection = (title, icon, content) =>
      ...
      const content = entries
        .map((entry) => {
          const dateRange = entry.current
            ? `*${entry.startDate} - Present*`
            : `*${entry.startDate} - ${entry.endDate}*`;
          // Sử dụng `\n\n` giữa dòng ngày tháng và mô tả để tạo khoảng cách đoạn.
          return `### **${entry.title}** at *${entry.organization}*\n${dateRange}\n\n${entry.description}`;
        })
        // Sử dụng `\n\n` để phân tách rõ ràng giữa các entry.
        .join("\n\n");
    ...
    // Sử dụng `\n\n---\n\n` để tạo đường kẻ ngang phân tách các section lớn.
    return [getContactMarkdown(), ...sections].join("\n\n---\n\n");
  };
  // === KẾT THÚC SỬA LỖI ===
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="space-y-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="preview">
            <div className="resume-container" data-color-mode="light">
              <MDEditor.Markdown
                source={previewContent}
